...
public class UniformFloat extends FloatProvider {
   public static final MapCodec<UniformFloat> CODEC = RecordCodecBuilder.mapCodec(
         i -> i.group(Codec.FLOAT.fieldOf("min_inclusive").forGetter(u -> u.minInclusive), Codec.FLOAT.fieldOf("max_exclusive").forGetter(u -> u.maxExclusive))
            .apply(i, UniformFloat::new)
      )
      .validate(
         u -> u.maxExclusive <= u.minInclusive
            ? DataResult.error(() -> "Max must be larger than min, min_inclusive: " + u.minInclusive + ", max_exclusive: " + u.maxExclusive)
