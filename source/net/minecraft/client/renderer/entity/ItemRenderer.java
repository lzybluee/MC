...
public class ItemRenderer {
   ...
   private static VertexConsumer getSpecialFoilBuffer(final MultiBufferSource bufferSource, final RenderType renderType, final PoseStack.Pose cameraPose) {
      return VertexMultiConsumer.create(
         new SheetedDecalTextureGenerator(
            bufferSource.getBuffer(useTransparentGlint(renderType) ? RenderTypes.glintTranslucent() : RenderTypes.glint()), cameraPose, 0.0078125F
         ),
   ...
   public static VertexConsumer getFoilBuffer(final MultiBufferSource bufferSource, final RenderType renderType, final boolean sheeted, final boolean hasFoil) {
      if (hasFoil) {
         return useTransparentGlint(renderType)
            ...
            : VertexMultiConsumer.create(bufferSource.getBuffer(sheeted ? RenderTypes.glint() : RenderTypes.entityGlint()), bufferSource.getBuffer(renderType));
      } else {
   ...
   public static List<RenderType> getFoilRenderTypes(final RenderType baseRenderType, final boolean sheeted, final boolean hasFoil) {
      if (hasFoil) {
         return useTransparentGlint(baseRenderType)
            ? List.of(baseRenderType, RenderTypes.glintTranslucent())
            : List.of(baseRenderType, sheeted ? RenderTypes.glint() : RenderTypes.entityGlint());
      } else {
