...
public class BlockRenderDispatcher implements ResourceManagerReloadListener {
   ...
   public void renderBreakingTexture(
      final BlockState state, final BlockPos pos, final BlockAndTintGetter level, final PoseStack poseStack, final VertexConsumer builder
   ) {
   ...
   public void renderLiquid(
      final BlockPos pos, final BlockAndTintGetter level, final VertexConsumer builder, final BlockState blockState, final FluidState fluidState
   ) {
      try {
         Objects.requireNonNull(this.liquidBlockRenderer).tesselate(level, pos, builder, blockState, fluidState);
      } catch (Throwable t) {
   ...
   public void renderSingleBlock(
      final BlockState state, final PoseStack poseStack, final MultiBufferSource bufferSource, final int lightCoords, final int overlayCoords
   ) {
      ...
      if (shape != RenderShape.INVISIBLE) {
         ...
         ModelBlockRenderer.renderModel(
            poseStack.last(), bufferSource.getBuffer(ItemBlockRenderTypes.getRenderType(state)), model, r, g, b, lightCoords, overlayCoords
         );
      }
   }
