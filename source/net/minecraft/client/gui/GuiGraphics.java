...
public class GuiGraphics {
   ...
   public void drawString(final Font font, final FormattedCharSequence str, final int x, final int y, final int color, final boolean dropShadow) {
      if (ARGB.alpha(color) != 0) {
         this.guiRenderState
            .submitText(new GuiTextRenderState(font, str, new Matrix3x2f(this.pose), x, y, color, 0, dropShadow, false, this.scissorStack.peek()));
      }
   }
            ...
            this.guiRenderState
               .submitItem(
                  new GuiItemRenderState(
                     itemStack.getItem().getName().toString(), new Matrix3x2f(this.pose), itemStackRenderState, x, y, this.scissorStack.peek()
                  )
               );
         } catch (Throwable t) {
      ...
      if (hoveredStyle != null) {
         if (hoveredStyle.getHoverEvent() != null) {
            switch (hoveredStyle.getHoverEvent()) {
               case HoverEvent.ShowItem(ItemStack item):
   ...
   public void submitSignRenderState(
      final Model.Simple signModel, final float scale, final WoodType woodType, final int x0, final int y0, final int x1, final int y1
   ) {
   ...
   public ActiveTextCollector textRendererForWidget(final AbstractWidget owner, final GuiGraphics.HoveredTextEffects hoveredTextEffects) {
      return new GuiGraphics.RenderingTextCollector(this.createDefaultTextParameters(owner.getAlpha()), hoveredTextEffects, null);
   }
   ...
   public ActiveTextCollector textRenderer(final GuiGraphics.HoveredTextEffects hoveredTextEffects) {
      return this.textRenderer(hoveredTextEffects, null);
   }

   public ActiveTextCollector textRenderer(
      final GuiGraphics.HoveredTextEffects hoveredTextEffects, final @Nullable Consumer<Style> additionalHoverStyleConsumer
   ) {
      return new GuiGraphics.RenderingTextCollector(this.createDefaultTextParameters(1.0F), hoveredTextEffects, additionalHoverStyleConsumer);
   }
   ...
   public enum HoveredTextEffects {
      ...
      HoveredTextEffects(final boolean allowTooltip, final boolean allowCursorChanges) {
         this.allowTooltip = allowTooltip;
         this.allowCursorChanges = allowCursorChanges;
      }

      public static GuiGraphics.HoveredTextEffects notClickable(final boolean canTooltip) {
   ...
   private class RenderingTextCollector implements ActiveTextCollector, Consumer<Style> {
      ...
      private RenderingTextCollector(
         ...
         final GuiGraphics.HoveredTextEffects hoveredTextEffects,
         final @Nullable Consumer<Style> additonalConsumer
      ) {
      ...
      public void accept(
         ...
         if (needsFullStyleScan) {
            ActiveTextCollector.findElementUnderCursor(renderState, GuiGraphics.this.mouseX, GuiGraphics.this.mouseY, this);
         }
      }
      ...
      public void acceptScrolling(
         ...
         int lineWidth = GuiGraphics.this.minecraft.font.width(message);
         int lineHeight = 9;
