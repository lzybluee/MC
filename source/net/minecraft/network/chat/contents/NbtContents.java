...
public class NbtContents implements ComponentContents {
   ...
   private NbtContents(
      final String nbtPathPattern,
      final NbtPathArgument.@Nullable NbtPath compiledNbtPath,
   ...
   public boolean equals(final Object o) {
      return this == o
         ? true
         : o instanceof NbtContents that
            && this.dataSource.equals(that.dataSource)
            && this.separator.equals(that.separator)
            && this.interpreting == that.interpreting
            && this.nbtPathPattern.equals(that.nbtPathPattern);
   }
   ...
   public int hashCode() {
      ...
      return 31 * result + this.dataSource.hashCode();
   }
      ...
      if (source != null && this.compiledNbtPath != null) {
         ...
         if (this.interpreting) {
            ...
            Component resolvedSeparator = (Component)DataFixUtils.orElse(
               ComponentUtils.updateForEntity(source, this.separator, entity, recursionDepth), ComponentUtils.DEFAULT_NO_STYLE_SEPARATOR
            );
            ...
            return ComponentUtils.updateForEntity(source, this.separator, entity, recursionDepth)
               .map(
                  resolvedSeparatorx -> stringElements.map(Component::literal)
                     .reduce((left, right) -> left.append(resolvedSeparatorx).append(right))
                     .orElseGet(Component::empty)
               )
               .orElseGet(() -> Component.literal(stringElements.collect(Collectors.joining(", "))));
         }
      } else {
   ...
   private static String asString(final Tag tag) {
