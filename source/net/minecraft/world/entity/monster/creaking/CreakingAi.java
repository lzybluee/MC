...
public class CreakingAi {
   ...
   static void initIdleActivity(final Brain<Creaking> brain) {
      brain.addActivity(
         Activity.IDLE,
         10,
         ImmutableList.of(
            StartAttacking.create(
               (level, creaking) -> creaking.isActive(), (level, creaking) -> creaking.getBrain().getMemory(MemoryModuleType.NEAREST_VISIBLE_ATTACKABLE_PLAYER)
            ),
   ...
   static void initFightActivity(final Creaking body, final Brain<Creaking> brain) {
   ...
   public static Brain<Creaking> makeBrain(final Creaking creaking, final Brain<Creaking> brain) {
