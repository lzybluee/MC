...
public class SimpleRegionStorage implements AutoCloseable {
   ...
   public SimpleRegionStorage(
      ...
      final Supplier<LegacyTagFixer> legacyFixer
   ) {
   ...
   public CompoundTag upgradeChunkTag(CompoundTag chunkTag, final int defaultVersion, final @Nullable CompoundTag dataFixContextTag) {
      ...
      try {
         chunkTag = this.legacyFixer.get().applyFix(chunkTag);
         injectDatafixingContext(chunkTag, dataFixContextTag);
         chunkTag = this.dataFixType.updateToCurrentVersion(this.fixerUpper, chunkTag, Math.max(this.legacyFixer.get().targetDataVersion(), version));
   ...
   public CompoundTag upgradeChunkTag(final CompoundTag chunkTag, final int defaultVersion) {
      return this.upgradeChunkTag(chunkTag, defaultVersion, null);
   }

   public Dynamic<Tag> upgradeChunkTag(final Dynamic<Tag> chunkTag, final int defaultVersion) {
      return new Dynamic(chunkTag.getOps(), this.upgradeChunkTag((CompoundTag)chunkTag.getValue(), defaultVersion, null));
   }
   ...
   protected void markChunkDone(final ChunkPos pos) {
      this.legacyFixer.get().markChunkDone(pos);
   }
